...
if(isset($_POST["hidech"]))
{
 ...
 if ($knowopt["opt_inv"]!=$value && $value == 1)//если включаем
 {
  if ($credits>=$servis["servis_credit"])
  {
   ...
   WriteLogs("Servises_",$_SESSION["user"]." спр¤тал в топе статы");
  }
 }
...
else
{
 ...
 if ($getcred>0)
 {
   ...
   WriteLogs("SMSDeluxe", $_SESSION["user"]." перешел на сайт платежной системы дл¤ плаежа, уникальный номер операции ".$uid[0]);
   if($uid[0])
   {
