...
{	
 ...
 if ($_REQUEST["do_greset"])
 {
  ...
  { 
   if ($gres["greset_reward"]==1)
   {
	...
	if ($i==$plase) $out_nitems.= $newitem;
     ...
     WriteLogs ("GReset_","Аккаунт ".$_SESSION["user"]." персонаж: ".$_SESSION["character"]." сейчас делает гресет, награда сгенерирована и положена в сундук. Hex:".$newitem);
   }
   else 
  ...
  elseif($gres["greset_reward"]==2)
  {
   ...
   WriteLogs ("GReset_","Аккаунт ".$_SESSION["user"]." персонаж: ".$_SESSION["character"]." сейчас делает гресет, награда ".$gres["credits_reward"]." credits");
  }
	
  if($db->query($queryres))
  {
   WriteLogs ("GReset_","Аккаунт ".$_SESSION["user"]." персонаж: ".$_SESSION["character"]." сделал гресет >".$gres["greset_reward"]);
