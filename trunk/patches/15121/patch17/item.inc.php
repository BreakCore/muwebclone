  ...
  **/
  public function readitems($hex,$itembd,$ancbd)
  {
   ...
   if($is32) //если вещи хотя бы 2го сезона
   {
    ...
    $result["group"] = floor(items::dehex($hex,18,2)/16);
    $excopt = items::dehex($hex,14,2);
    $result["dur"] = items::dehex($hex,4,2);

	if ($result["intopt"]>0 || $excopt>0) $result += items::getOptions($result["group"],$result["id"],$result["intopt"],$excopt);

	$result["equipment"] =items::showEq($result["group"],$result["id"],$itembd);
	$result = items::GetName($result,$itembd);
	
	if ($result["name"] && strlen($result["name"])<28)
	{
	 if (items::dehex($hex,19,1)>0) $lvl380 = "Additional Dmg+200<br>Pow Success Rate +10";
     else $lvl380 = "";
	 ...
	 if ($ancient>0) 
	 {
	  $result += items::isAncient($result["group"],$result["id"],$ancient,$ancbd);
	 ...
	 if ($harmony>0 && $hlvl>0)
     {
	  $result["harmony"] =items::GetHarmony($result["group"],$harmony,$hlvl);
 ...
 private function Sockets($hex)
 {
  ...
  if ($hex!="FFFFFFFFFF" or $hex!="0000000000") 
  {
   $socket = array(
		...
		$i=0;
		 $sockets[] = "<br><span style=color:#B575D1;font-size:11px;font-weight:bold;>Socket System</span>";

		while ($i<10)
		{
		 if ($socket[substr($hex,$i,2)]!="") $sockets[].="<br><span style=color:#9400D3;font-size:9px;font-weight:bold;>".$socket[substr($hex,$i,2)]."</span>";
			$i+=2;
		}
		if (count($sockets)<2 ) unset($sockets);
			}
	$ss["sokets"] = $sockets;
	return $ss;
 }
 ...
 public function GetName($opt,$itembd)
 {
  ...
  if ($opt["group"]==13 && $opt["id"]==31) 
	...
	elseif($opt["group"]==13)
	{
	  switch($opt["id"])
	  {
	    case 7: if($opt["level"]==1){ $opt["name"]="Sperman";$opt["elevel"] = $opt["level"];unset($opt["level"],$opt["equipment"]);$opt["llevel"]=7;} break;
	    case 11: if($opt["level"]==1){ $opt["name"]="Life Stone";$opt["elevel"] = $opt["level"];unset($opt["level"],$opt["equipment"]);$opt["llevel"]=7;}
		         else { $opt["name"]="Guardian";$opt["elevel"] = $opt["level"];unset($opt["level"],$opt["equipment"]);$opt["llevel"]=7;} break;
	    case 14: if($opt["level"]==1){ $opt["name"]="Crest of Monarch";$opt["elevel"] = $opt["level"];unset($opt["level"],$opt["equipment"]);$opt["llevel"]=7;} break;
	  }
	}
	elseif($opt["group"]==14)
	{
	  switch($opt["id"])
	  {
	    case 7: if($opt["level"]==1) { $opt["name"]="Potion of Soul";$opt["elevel"] = $opt["level"];unset($opt["level"],$opt["equipment"]);$opt["llevel"]=7;} break;
	    case 12: if($opt["level"]==1){ $opt["name"]="Heart";$opt["elevel"] = $opt["level"];unset($opt["level"],$opt["equipment"]);}elseif($opt["level"]==2){ $opt["name"]="Pergamin";$opt["llevel"]=7;unset($opt["level"],$opt["equipment"]);} break;
	    case 21: if($opt["level"]==1){ $opt["name"]="Stone";$opt["llevel"]=7;$opt["elevel"] = $opt["level"];unset($opt["level"],$opt["equipment"]);}elseif($opt["level"]==3){ $opt["name"]="Sing of Lord";$opt["llevel"]=7;unset($opt["level"],$opt["equipment"]);} break;
	    case 32: if($opt["level"]==1){ $opt["name"]="Pink Candy Box";$opt["elevel"] = $opt["level"];unset($opt["level"],$opt["equipment"]);} break;
	    case 33: if($opt["level"]==1){ $opt["name"]="Orange Candy Box";$opt["elevel"] = $opt["level"];unset($opt["level"],$opt["equipment"]);} break;
	    case 34: if($opt["level"]==1){ $opt["name"]="Blue Candy Box";$opt["elevel"] = $opt["level"];unset($opt["level"],$opt["equipment"]);} break;
	  }
	}
	if (empty($opt["name"]))
	{
 ...
 **/
 ...
 private function isExcelent ($group,$id,$ex=0)
 {
  ...
  {				
	 ...
	 elseif($group==13 && $id==37)
	 {
	  switch ($ex)
	  {
	   case 1: $Excellent[0] = "<div class=\"excellent\" >Plazma Storm Skill</div><div class=\"excellent\" >Increase final damage 10%</div>"; break;
	   case 2: $Excellent[0] = "<div class=\"excellent\" >Plazma Storm Skill</div><div class=\"excellent\" >Absorb final damage 10%</div>"; break;
	   case 4: $Excellent[0] = "<div class=\"excellent\" >Plazma Storm Skill</div><div class=\"excellent\" >Increase final damage 10%</div><div class=\"excellent\" >Absorb final damage 10%</div>"; break;
	  }
	  $ex=0;
	 }
...
class Iitems
{
  ...
  * @action - текст с жс эвентом
  **/
  public function showWH($hex,$link="",$action="")
  { 
	...
	for ($i=0;$i<$col_i;$i++) 
    {
	 ...
	 $img = "imgs/items/".$items["group"].$items["id"].$items["intopt"].".gif"; // looking for item img
	   ...
	   if (!file_exists($img))$img="<span style='font-size:8px;'>".$items["group"].".".$items["id"].".".$items["intopt"]."</span>"; //если нет изображения
	    ...
	    if ($action!="")$actions=" Onclick='".$action."(items".$i.");return false;'";
		
	 while($j>0)
	   {
		if($x1<=$x)
		{
		 if (!empty($link)) $link1="<a href='".$link."=".$i."' ".$actions." >$img</a>";
		 else $link1=$img;
		 
	     if ($str==$i)$itemarr[$str]="<td align='center'  colspan='".$x."' rowspan='".$y."' style='color:white;background:url(imgs/2.png);width:32px;height:32px;' id='items".$i."'>".$link1."</td>";
  ...
  /**
  * возвращает сформированную таблицу опций
  ...
  * 5 = показывает изоюражение, при наветедии полностью все опции
  * 6 = краткая информация о вещи в строку
  **/
  public function getOptions($item,$type=0,$itemdb="")
  {
   if (!is_array($item)) return "wrong item format!";
   if ($item["hex"]=="FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF") return "";
	...
	$randp = array(8,9,12,13,20,21,22,23,24,25,26,27,28);
	$excl = $item["excellent"];
	if ((is_array($excl) && $item["group"]<12 )|| ($item["group"]==13 && in_array($item["id"],$randp)))
	{
	 $csstyle="name_ex";
	...
	Switch($type)
	{
	 case 0:
	  ...
	  if ($item["dur"]) $display.="<tr><td align=\"center\" valign=\"center\" class=\"$eqtcl\">Durability: [".$item["dur"]."]</td></tr>";
	...
	case 3:
	 ...
	 $n = $item["name"].$item["level"];
	  ...
	  if ($item["dur"]) $display1.="<tr><td align=\"center\" valign=\"center\" class=\"$eqtcl\">Durability: [".$item["dur"]."]</td></tr>";
	  if ($item["skill"])
      {
	 ...
	 $n = $item["name"].$item["level"];
	  ...
	  $x = substr($itemdb[$item["group"]][$item["id"]][1],0,1);
	  $y = substr($itemdb[$item["group"]][$item["id"]][1],1,1);
	  //return $x."-".$y;
      $display = "<table border='0' style='width:".($x*32)."px;height:".($y*32)."px;'>	  
	  <tr><td align=\"center\" id='tooltiper' title='$display1' valign=\"center\" >".$img."</td></tr>
      </table>"; 
	 break;
	 case 6:
	  if ($item["level"]==0)unset($item["level"]);
