...
function login($type="open")
{
 ...
 if ($_REQUEST["usrout"])
 {
  ...
  unset($_SESSION["user"],$_SESSION["pwd"],$_SESSION["character"],$_SESSION["adm"]); 
  ...
  { 
   if ($config["ucapch"]==1)
   {
     ...
     $queryregNum = $db->numrows($db->query("SELECT memb___id, memb__pwd FROM MEMB_INFO WHERE memb___id ='".$userl."' and memb__pwd = ".$suserp.""));
	  ...
	  if (!isset($_SESSION["sadmin"]) || !isset($_SESSION["spwd"])) unset($_SESSION["adm"]);

   if ($validadm==0 && $queryregNum==0)
   {
    unset($_SESSION["sadmin"],$_SESSION["spwd"],$_SESSION["adm"]);
	header("Location:".$config["siteaddress"]);
	die();
   }
     if ($_SESSION["adm"]==1 && $queryregNum == 0 && $_SESSION["sadmin"] && $_SESSION["spwd"])
     {
    	 header("Location:".$config["siteaddress"]."/control.php");
		 die();
	 }
     else if ($validadm == 1 && !$_SESSION["adm"])
     {  
